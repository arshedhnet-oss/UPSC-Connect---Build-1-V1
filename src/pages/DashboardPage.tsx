...
const DashboardPage = () => {
  ...
  const addSlot = async () => {
    ...
    if (error) {
      toast({ title: "Failed to add slot", description: error.message, variant: "destructive" });
    } else if (data) {
  ...
  const totalEarnings = profile.role === "mentor"
    ? completedSessions.length * (mentorProfile?.price_per_session || 0)
    : 0;
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <Card>
            <CardContent className="pt-6">
              ...
              <p className="text-3xl font-display font-bold text-foreground">{upcomingSessions.length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
          ...
          {profile.role === "mentor" && (
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-3xl font-display font-bold text-accent">₹{totalEarnings}</p>
              </CardContent>
            </Card>
          )}
        </div>
          ...
          <CardContent>
            {bookings.length === 0 ? (
              <p className="text-muted-foreground">
                {profile.role === "mentee" ? (
                  <>No bookings yet. <Link to="/mentors" className="text-primary hover:underline">Browse mentors</Link> to get started.</>
                ) : "No sessions yet."}
              </p>
            ) : (
                  ...
                  <div key={b.id} className="flex items-center justify-between rounded-lg border border-border p-4">
                    <div>
                      <p className="font-medium text-foreground">
                        {profile.role === "mentee" ? `Mentor: ${b.mentor?.name}` : `Mentee: ${b.mentee?.name}`}
                      </p>
                      {b.slots && (
