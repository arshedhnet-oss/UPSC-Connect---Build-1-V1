...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check if mentor has already sent any message (real or auto)
    ...
    if (mentorMessages && mentorMessages.length > 0) {
      return new Response(
        JSON.stringify({ skipped: true, reason: "mentor_already_messaged" }),
    ...
    console.log(`Auto-response sent for conversation ${conversation_id}`);
